    ...
    <div class="server-list" id="serverList">
        ...
        <?php else: ?>
            <?php foreach ($servers as $i => $server): 
                    ...
                    $bgColor = ($server['has_bg_color'] && $server['highlight_color']) ? 'background: linear-gradient(135deg, '.e($server['highlight_color']).'22 0%, var(--bg-card) 100%) !important;' : '';
                    $borderColor = ($server['has_border'] && $server['highlight_color']) ? 'border: 1px solid '.e($server['highlight_color']).' !important; box-shadow: 0 0 15px '.e($server['highlight_color']).'33 !important;' : '';
...
(function() {
    ...
    function renderServerCard(s, rank) {
        ...
        if (s.highlight_color) {
            let styles = [];
            if (s.has_bg_color == 1 || s.has_bg_color === true) styles.push('background: linear-gradient(135deg, ' + escapeHtml(s.highlight_color) + '22 0%, var(--bg-card) 100%) !important');
            if (s.has_border == 1 || s.has_border === true) {
                styles.push('border: 1px solid ' + escapeHtml(s.highlight_color) + ' !important');
                styles.push('box-shadow: 0 0 15px ' + escapeHtml(s.highlight_color) + '33 !important');
            }
